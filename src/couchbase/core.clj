...
(defn validate-opts
  ...
  (when (= (:workload opts) "disk-failure")
    (if-not (:manipulate-disks opts)
      (throw (RuntimeException. "disk-failure workload requires --manipulate-disks option")))
    (if (not= (:cycles opts 1) 1)
      (throw (RuntimeException. "disk-failure workload only supports a single \"cycle\""))))
